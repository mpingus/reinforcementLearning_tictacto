...
class Board():
    ...
    def __init__(self, board=None, currentBoard=None):
        # define players
        self.player_1 = 'x'
        self.player_2 = 'o'
        self.empty_square = '.'
        ...
        # init (reset) board
        if currentBoard is not None:
            self.inputTransformator(currentBoard)
        else: self.init_board()
    ...
    def inputTransformator(self, inputBoard):
        for row in range(3):
            for col in range(3):
                ...
                if inputBoard[row, col] == 1: 
                    self.position[row, col] = 'x'
                
                if inputBoard[row, col] == -1: 
                    self.position[row, col] = 'o'
    
    def outputTransformator(self, best_move):
        for row in range(3):
            for col in range(3):
                if self.position[row, col] != best_move[row, col]:
                    return row, col 
